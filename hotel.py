...
def get_hotel_prices(name: str, checkin_date: str, checkout_date: str) -> List[Dict]:
    ...
    """
    print(f"Getting prices of {name} between {checkin_date} and {checkout_date}")
    ...
    time.sleep(3)
    try:
        driver.find_element(By.CSS_SELECTOR, f'[aria-label="{name}"]').click()
        time.sleep(5)
    except NoSuchElementException:
        pass
    try:
        ...
        print(f"設定日期失敗: {name}-{checkin_date}-{checkout_date}")
        get_screenshot(driver, f"設定日期失敗-{name}-{checkin_date}-{checkout_date}")
        return []
    except ElementNotInteractableException:
        print(f"設定日期失敗: {name}-{checkin_date}-{checkout_date}")
        get_screenshot(driver, f"設定日期失敗-{name}-{checkin_date}-{checkout_date}")
    ...
    # 查看更多選項
    open_all_options(driver=driver)

    file_name: str = f"{name}_{checkin_date}_{checkout_date}.png"
    get_screenshot(driver, file_name)
    try:
        all_options = [
...
def main():
    ...
    if options.from_csv_file is None:
        ...
        for row in reader.to_dict(orient="records"):
            ...
            hotel = row['name']
            print(f"Getting prices of {hotel} between {row['start']} and {row['end']}")
            data = {}
            while start < end:
